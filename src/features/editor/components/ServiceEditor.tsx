...
import type { ComposeResource, ComposeService, ValidationIssue } from "../../../models/composeTypes";
...
export const ServiceEditor = ({
    ...
    onUpdate: (service: ComposeService) => void;
    ...
    const update = (field: string, value: unknown) => onUpdate({ ...service, [field]: value });
    ...
    const updateNested = (path: string, value: unknown) => {
        ...
        let obj: Record<string, unknown> = newService;
    ...
    const getFieldError = (patterns: string[]): ValidationIssue | undefined => {
        return errors.find(
            (e) => e.name === name && patterns.some((p) => e.message.toLowerCase().includes(p.toLowerCase())),
        );
    };
    ...
    return (
        <div className="space-y-4 animate-slide-in">
            ...
            <div className="flex items-center justify-between pb-3 border-b border-border/50">
                ...
                <div className="flex items-center gap-2">
                    {serviceErrors.length > 0 && (
                        <Badge type="error">
                            {serviceErrors.length} issue{serviceErrors.length !== 1 && "s"}
                        </Badge>
                    )}
            ...
            >
                <Input
                    label="Command"
                    value={Array.isArray(service.command) ? service.command.join(" ") : service.command}
                ...
                <Input
                    label="Entrypoint"
                    value={Array.isArray(service.entrypoint) ? service.entrypoint.join(" ") : service.entrypoint}
            ...
            <Section
                title="Networking"
                icon={Globe}
                defaultOpen={Boolean(portsError) || Boolean(networksError)}
            >
                <ArrayEditor
                    label="Ports"
                    value={service.ports}
                ...
                <ArrayEditor
                    label="Expose"
                    value={service.expose}
                    onChange={(v) => update("expose", v)}
                    placeholder="3000"
                />
                <div className="space-y-2">
                    <label
                        className={`text-xs ${networksError ? "text-error flex items-center gap-1" : "text-text-secondary"}`}
                    >
                    ...
                    <div className="flex flex-wrap gap-2">
                        {Object.keys(allNetworks).map((net) => (
                            <label
                                key={net}
                                className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-surface-raised/50 cursor-pointer hover:bg-surface-raised"
                            >
                                <input
                                    type="checkbox"
                                    checked={normalizeArray(service.networks).includes(net)}
                                    onChange={(e) =>
                                        update(
                                            "networks",
                                            e.target.checked
                                                ? [...normalizeArray(service.networks), net]
                                                : normalizeArray(service.networks).filter((n) => n !== net),
                                        )
                                    }
                                    className="rounded"
                                />
                            ...
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1 space-y-2 text-sm">
                                    ...
                                    <p className="text-text-secondary text-xs">
                                        {getErrorHelp(networksError.message).explanation}
                                    </p>
                                    <div className="flex items-start gap-2 p-2 bg-success/10 rounded border border-success/30">
                                        ...
                                        <p className="text-xs text-success">
                                            {getErrorHelp(networksError.message).solution}
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            >
                <ArrayEditor
                    label="Env Files"
                    value={normalizeArray(service.env_file)}
            ...
            <Section
                title="Volumes"
                icon={Database}
                defaultOpen={Boolean(volumesError)}
            >
                <ArrayEditor
                    label="Volume Mounts"
            ...
            <Section
                title="Dependencies"
                icon={Layers}
                defaultOpen={Boolean(dependsOnError)}
            >
                <div className="space-y-2">
                    <label
                        className={`text-xs ${dependsOnError ? "text-error flex items-center gap-1" : "text-text-secondary"}`}
                    >
                        ...
                        {Object.keys(allServices)
                            ...
                            .map((svc) => (
                                ...
                                >
                                    <input
                                        type="checkbox"
                                        checked={normalizeDependsOn(service.depends_on).includes(svc)}
                                        onChange={(e) =>
                                            update(
                                                "depends_on",
                                                e.target.checked
                                                    ? [...normalizeDependsOn(service.depends_on), svc]
                                                    : normalizeDependsOn(service.depends_on).filter((d) => d !== svc),
                                            )
                                        }
                                        className="rounded"
                                    />
                        ...
                        <div className="mt-2 p-3 rounded-lg border border-error/40 bg-error/10 animate-fade-in">
                            <div className="flex items-start gap-2">
                                ...
                                <div className="flex-1 space-y-2 text-sm">
                                    ...
                                    <p className="text-text-secondary text-xs">
                                        {getErrorHelp(dependsOnError.message).explanation}
                                    </p>
                                    <div className="flex items-start gap-2 p-2 bg-success/10 rounded border border-success/30">
                                        <CheckCircle
                                            size={14}
                                        ...
                                        />
                                        <p className="text-xs text-success">
                                            {getErrorHelp(dependsOnError.message).solution}
                                        </p>
                                    </div>
                                </div>
                            </div>
            ...
            >
                <Input
                    label="Test Command"
                    value={Array.isArray(healthcheck.test) ? healthcheck.test.join(" ") : healthcheck.test}
                    onChange={(v) => updateNested("healthcheck.test", v.split(" "))}
